...
Path scheme: Lab prefixes (/rps/*, /euclidorithm/*, /polyfolding/*).
...
Gradually normalize geometry/euclidorithm and geometry/polyfolding to the same shell conventions used by RPS.
...
aix_web/labs/polyfolding_adapter.py (initial placeholder + data/training module bridge).
geometry/euclidorithm/* (existing code retained initially, wrapped).
geometry/polyfolding/* (existing/ongoing data-generation assets retained initially).
...
Phase 3: Add geometry/polyfolding wrapper with placeholder web lab + data hooks.
